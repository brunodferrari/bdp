...
X_balan = X#, y_balan =  undersample.fit_resample(X, best_mh_labeled)

y_balan = best_mh_labeled
...
acc_svm = cross_val_score(svc_grid, X_balan, y_balan, n_jobs=5, verbose=1, scoring='accuracy')
print('Accuracy')
print(round(acc_svm.mean(), 2))
print(round(acc_svm.std(), 2))
roc_svm = cross_val_score(svc_grid,  X_balan, y_balan, n_jobs=5, verbose=1, scoring='roc_auc_ovr')
print('ROC')
print(round(roc_svm.mean(), 2))
print(round(roc_svm.std(), 2))
svm_hat = cross_val_predict(svc_grid,  X_balan, y_balan, n_jobs=-1, verbose=1)
...
acc_ad = cross_val_score(ad_grid, X_balan, y_balan, n_jobs=5, verbose=1, scoring='accuracy')
print('Accuracy')
print(round(acc_ad.mean(), 2))
print(round(acc_ad.std(), 2))
print('ROC')
roc_ad = cross_val_score(ad_grid,  X_balan, y_balan, n_jobs=5, verbose=1, scoring='roc_auc_ovr')
print(round(roc_ad.mean(), 2))
print(round(roc_ad.std(), 2))
ad_hat = cross_val_predict(ad_grid,  X_balan, y_balan, n_jobs=-1, verbose=1)
...
acc_rf = cross_val_score(rf_grid, X_balan, y_balan, n_jobs=5, verbose=1, scoring='accuracy')
print('Accuracy')
print(round(acc_rf.mean(), 2))
print(round(acc_rf.std(), 2))
print('ROC')
roc_rf  = cross_val_score(rf_grid, X_balan, y_balan, n_jobs=5, verbose=1, scoring='roc_auc_ovr')
print(round(roc_rf.mean(), 2))
print(round(roc_rf.std(), 2))
rf_hat = cross_val_predict(rf_grid, X_balan, y_balan, n_jobs=-1, verbose=1)
...
acc_knn = cross_val_score(knn_grid, X_balan, y_balan, n_jobs=5, verbose=1, scoring='accuracy')
print('Accuracy')
print(round(acc_knn.mean(), 2))
print(round(acc_knn.std(), 2))
print('ROC')
roc_knn  = cross_val_score(knn_grid, X_balan, y_balan, n_jobs=5, verbose=1, scoring='roc_auc_ovr')
print(round(roc_knn.mean(), 2))
print(round(roc_knn.std(), 2))
knn_hat = cross_val_predict(knn_grid, X_balan, y_balan, n_jobs=-1, verbose=1)
...
for var in globals():
    ...
    if str(var).find("_hat") > 0:
        #pos_ = pos.pop(0)
        ConfusionMatrixDisplay(confusion_matrix(y_balan, globals()[var]), ['VND/TABU', 'GRASP']).plot()
