...
X = dados_scatter.drop(['V1', 'V2'],axis=1)
...
best_mh_labeled = best_mh.replace({'Crossing_ts': 3, 'Crossing_vns': 2, 'Crossing_gs_vns': 1}).fillna(1).astype(int)
...
for col in dados_scatter:
    pd.plotting.scatter_matrix(pd.concat([dados_scatter[col], best_mh], axis=1))
    plt.show()
...
for cols in X_Y:
    ax=pd.concat([dados_scatter[list(cols)], wbest_mh_labeled], axis=1).plot(kind='scatter', x=cols[0], y=cols[1], c=0, cmap="viridis", sharex=False,
                                                                          title='Ponderado', label=crossing_mh)
    plt.legend(handles = ax.legend_.legendHandles[0].legend_elements()[0], 
               labels=crossing_mh)
  #  plt.show()    

svc_mh = SVC(kernel='linear')
...
print(mod, mod.mean())
print(cross_val_predict(svc_grid, X, best_mh_labeled, n_jobs=-1, verbose=1))
