...
output_list = []
for i in range(1010)[::10]:
    with ThreadPoolExecutor(6) as ex:
        output = list(ex.map(lambda x: gera_grafo_2((_path+"bdp/dbdp_instances/stallman_reduced/{name}.txt").format(name=x)), list(df_results['Instance'][i-10:i])))
    with ThreadPoolExecutor(6) as ex:
        ex.map(lambda x: gera_txt(x[0], x[1]), zip(df_results['Instance'][i-10:i], output))
    output_list.append(pd.DataFrame(gera_features(output)))
    if not(i % 100):
        with pd.ExcelWriter(_path+"bdp/dbdp_instances/metafeat.xlsx") as writer:
            pd.concat([df_results, pd.concat(output_list, axis=0).reset_index(drop=True)],axis=1).to_excel(writer, sheet_name="results", index=False)
            #pd.concat(output_list, axis=0).reset_index(drop=True)
