...
for i in range(130)[::10]:
    with ThreadPoolExecutor(6) as ex:
         output = list(ex.map(lambda x: gera_grafo((_path+"bdp/dbdp_instances/instances/{name}.txt").format(name=x)), list(df_results['Instance'][i-10:i])))
    with ThreadPoolExecutor(6) as ex:
    ...
    if not(i % 20):
        with pd.ExcelWriter(_path+"bdp/dbdp_instances/metafeat2.xlsx") as writer:
